...
def main():
    ...
    parser.add_argument(
        "--num_files", type=int, default=500, help="Number of entry_ids to use."
    )
    parser.add_argument(
        "--input_format",
        type=str,
        default="csv",
        help="Input format for the previously-stored runs.",
    )
    parser.add_argument(
        "--output_format", type=str, default="csv", help="Output format."
    )
    parser.add_argument(
        "--continuous", action="store_true", help="Whether to keep iterating or not..."
    )
    parser.add_argument(
        "--split_sentences",
        action="store_true",
        help="Whether to just perform sentence-splitting.",
    )
    parser.add_argument(
        "--env",
        type=str,
        default="bb",
        help="Whether we're running on Bloomberg or somewhere else.",
    )
    ...
    if args.env == "bb":
        prefetched_entry_ids = sug.download_prefetched_data(
            args.db_name, split_sentences=args.split_sentences
        )
    else:
        prefetched_entry_ids = sug.read_prefetched_data(
            args.db_name, split_sentences=args.split_sentences
        )
    # else:
    ...
    if args.env == "bb":
        prefetched_file_idx, last_one, left_to_fetch_df = sug.download_pq_to_df(
            args.db_name, prefetched_entry_ids
        )
    else:
    ...
    if args.env == "bb":
        spark = (
            SparkSession.builder.config(
                "spark.jars.packages", "com.johnsnowlabs.nlp:spark-nlp_2.11:2.7.5"
            )
    ...
    num_tries = 3
    s3_path = (
        sug.s3_output_dir_main
        if not args.split_sentences
        else sug.s3_output_dir_sentences
    )
    file_count = (
        len(sug.get_files(s3_path, args.db_name, sug.csv_pat))
        if not args.split_sentences
        else len(sug.get_files(s3_path, args.db_name, sug.pkl_pat))
    )

    print("FILES FOUND: %s" % file_count)
    print(
        "NUM IDs FOUND: %s" % len(prefetched_entry_ids)
    ...
    while (len(left_to_fetch_df) > 0) or (not last_one):
        ...
        if len(to_fetch_this_round_df) == 0:
            ...
            prefetched_file_idx, last_one, left_to_fetch_df = sug.download_pq_to_df(
                args.db_name, prefetched_entry_ids, prefetched_file_idx
            )
            continue
        ...
        print(
            ...
            % ", ".join(
                list(
                    map(
                        str,
                        to_fetch_this_round_df["entry_id"].drop_duplicates().tolist(),
                    )
                )
            )
        )
        ...
        if args.split_sentences:
            output_sdf = sus.run_spark_sentences(
                to_fetch_this_round_df, spark, *pipelines
            )
        else:
        ...
        if len(output_df) == 0:
            # if num_tries > 0:
            print(
                "ZERO-LEN DF, getting next pretrained_file (pretrained_idx: %s)..."
                % prefetched_file_idx
            )
            #     num_tries -= 1
            #     continue
