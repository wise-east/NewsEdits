...
contains = ["Want to get this briefing by email", "Thank youTo"]
ends_with = [
    "]",
]
...
filter_sents = lambda x: not (
    any(map(lambda y: y in x, contains))
    or any(map(lambda y: x.startswith(y), starts_with))
    or any(map(lambda y: x.endswith(y), ends_with))
)
...
filter_stopword_lemmas = lambda word_list: list(
    filter(lambda x: x not in stopwords_lemmas, word_list)
)
filter_punct = lambda word_list: list(
    filter(lambda x: x not in string.punctuation, word_list)
)
...
# Get diffs.
#
def get_sentence_diff(
    a_old, a_new, filter_common_sents=True, merge_clusters=True, slack=0.5
):
    ## split sentences
...
def get_word_diffs(s_old, s_new, split_method="spacy"):
    s_old_words, s_new_words = get_words(s_old, split_method), get_words(
        s_new, split_method
    )
    ...
    for idx, item in enumerate(diffs):
        ...
        if label == "?":
            ...
            if (idx < len(diffs) - 1) and (diffs[idx + 1][0] == "+"):
                ...
                if get_word_diff_ratio(text, text_new) > 0.8:
                    in_question = True
                    continue

            vars_new.append({"text": "", "tag": " "})

        elif label == "+":
            vars_new.append({"text": text, "tag": "+"})
            if in_question:
                in_question = False
            else:
                vars_old.append({"text": "", "tag": " "})
...
def check_subset(s1_lemmas, s2_lemmas, slack=0.5):
...
def text_in_interval(c, idx_i, idx_j, version):
    ...
    return any(
        map(
            lambda idx: c[idx][version]["text"].strip() != "",
            range(idx_small + 1, idx_large),
        )
    )
        ...
        for active_version in [0, 1]:
            ...
            for idx_i, idx_j in itertools.product(r_c, r_c):
                ...
                if (
                    ...
                    s1_lemmas, cache = lemmatize_sentence(
                        c[idx_i][inactive_version]["text"], cache
                    )
                    s2_lemmas, cache = lemmatize_sentence(
                        c[idx_j][active_version]["text"], cache
                    )
                    if check_subset(s1_lemmas, s2_lemmas, slack=slack):
                        # if there's a match, first check:
                        combined_text_active = merge_sents(
                            idx_i, idx_j, active_version, c
                        )
                        combined_text_inactive = merge_sents(
                            idx_i, idx_j, inactive_version, c
                        )
                        ...
                        elif text_in_interval(
                            c, idx_i, idx_j, active_version
                        ) and text_in_interval(c, idx_i, idx_j, inactive_version):
                            # print('2.')
                            pass
                        ...
                        #   5. if there's no text inbetween the idx's in either the active or the inactive, move the active.
                            ...
                            text_in_interval(c, idx_i, idx_j, active_version)
                            and text_in_interval(c, idx_i, idx_j, inactive_version)
                        ):
                            # print('5.')
                            c = swap_text_spots(
                        ...
                        ## merge list/text
                        for idx, version in itertools.product(
                            [idx_i, idx_j], [active_version, inactive_version]
                        ):
                            if isinstance(c[idx][version]["text"], list):
                                c[idx][version]["text"] = merge_sents_list(
                                    c[idx][version]["text"]
                                )
...
def merge_all_clusters(vo, vn, slack=0.5):
...
def get_sentence_diff_stats(article_df, get_sentence_vars=False, output_type="df"):
    ...
    for a_id in tqdm(sample_ids):
        ...
        if output_type == "iter":
            yield from get_sentence_diff_stats_on_article_gen(
                a, a_id, get_sentence_vars
            )
        else:
            sentence_stats, word_stats = get_sentence_diff_stats_on_article(
                a, a_id, "df", get_sentence_vars
            )
...
def get_sentence_diff_stats_on_article_gen(a, get_sentence_vars, a_id=None):
    ...
    for v_old, v_new in list(zip(vs[:-1], vs[1:])):
        try:
            vars_old, vars_new = get_sentence_diff(
                a_by_v.loc[v_old]["summary"], a_by_v.loc[v_new]["summary"]
            )
        ...
        if vars_old is None and vars_new is None:
            ...
            sentence_stat_output = {
                ...
                "num_changed_sents": len(
                    doc_changes["sentences"]["changed_sent_pairs"]
                ),
...
def html_compare_articles(
    ...
    if vars_old is None and vars_new is None:
        if (
            text_old is None
            and text_new is None
            and tags_old is None
            and tags_new is None
        ):
            df = df.sort_values("s_idx")
            text_old, text_new, tags_old, tags_new = (
                df["sent_old"],
                df["sent_new"],
                df["tag_old"],
                df["tag_new"],
        ...
        vars_old = list(
            map(lambda x: {"text": x[0], "tag": x[1]}, zip(text_old, tags_old))
        )
        vars_new = list(
            map(lambda x: {"text": x[0], "tag": x[1]}, zip(text_new, tags_new))
        )

    html = ["<table>", "<tr><th>Old Version</th><th>New Version</th></tr>"]
    ...
    for s_old, s_new in zip(vars_old, vars_new):
        ...
        ##
        if s_old["tag"] == "-" and s_new["tag"] == "+":
            s_old_text, s_new_text = html_compare_sentences(
                *get_word_diffs(s_old_text, s_new_text)
            )
        ##
        if s_old["tag"] == "-":
            row += (
                '<td style="background-color:rgba(255,0,0,0.3)">' + s_old_text + "</td>"
            )
        else:
        ...
        if s_new["tag"] == "+":
            row += (
                '<td style="background-color:rgba(0,255,0,0.3)">' + s_new_text + "</td>"
            )
        else:
...
def compare_sentences_output(
    ...
    if isinstance(old_sent, str) and isinstance(new_sent, str):
        old_sent, new_sent = get_word_diffs(
            old_sent, new_sent, split_method=split_method
        )
...
def latex_compare_sentences(
    old_sent, new_sent, split_method="spacy", return_dict=False
):
