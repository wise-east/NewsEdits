...
if predictions_fp.endswith(".csv"):
    ...
    test_fp = "/project/jonmay_231/hjcho/NewsEdits/prediction/sentence_only_test.jsonl"
...
overall_accuracy = sum([1 if label == prediction else 0 for label, prediction in zip(labels, predictions)]) / len(labels)
...
for category in ["fact", "style", "none"]: 
    ...
    category_predictions = [1 if prediction == category else 0 for prediction in predictions]
    category_accuracy = sum([1 if label == category and prediction == category else 0 for label, prediction in zip(labels, predictions)]) / sum(category_labels)
...
results_columns = [
    category_f1_scores["fact"], 
    category_f1_scores["style"], 
